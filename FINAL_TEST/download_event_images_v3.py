...
for event in events:
    ...
    # Use exact event name without 2026 to find historical/actual photos
    query = event_name.replace('2026', '').strip()
    print(f"[{event_id}] Downloading 1 image for '{query}'...")
    
    temp_dir = "temp_bing_images_2"
    try:
        # filter_type='photo' helps get actual photos instead of logos/clipart
        downloader.download(query, limit=1, output_dir=temp_dir, adult_filter_off=False, force_replace=False, timeout=10, verbose=False, filter="photo")
        ...
        if files:
            ...
            new_filename = f"event_real2_{event_id}{ext}"
...
if os.path.exists("temp_bing_images_2"):
    shutil.rmtree("temp_bing_images_2")
